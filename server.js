...
app.get("/api/quizzes/:id/responses", (req,res)=>{
  ...
  const responses = rows.map(r=>({
    ...
    answers:JSON.parse(r.answers_json).map((a,i)=>({question:questions[i]?.text || "",answer:questions[i]?.options?.[a] || "—",correct:questions[i]?.options?.[questions[i]?.correct] || "—",isCorrect:Number(a)===questions[i]?.correct}))
  }));
...
app.get("/api/health", (req,res)=>res.json({ok:true,service:"VibeMeter"}));
