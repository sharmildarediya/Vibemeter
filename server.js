...
app.post("/api/quizzes/:id/responses", (req,res)=>{
  ...
  res.json({ok:true, score:percent, review:questions.map((q,i)=>({question:q.text, selected:q.options[Number(answers[i])]||"—", correct:q.options[q.correct]||"—", correctIndex:q.correct, selectedIndex:Number(answers[i]), isCorrect:Number(answers[i])===q.correct}))});
});
